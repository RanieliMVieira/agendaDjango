...
def cadastro(request):
    ...
    if not nome or not sobrenome or not email or not usuario or not senha \
            or not repetir_senha:
        messages.error(request, 'Os campos não podem serem vazios!')
        return render(request, 'accounts/cadastro.html')

    try:
        validate_email(email)
    except:
        messages.error(request, 'e-mail inválido, favor informar um correto!')
        return render(request, 'accounts/cadastro.html')

    if len(senha) < 6:
        messages.error(request, 'senha precisa ser maior que 6 caracteres ou mais!')
        return render(request, 'accounts/cadastro.html')

    if len(usuario) < 6:
        messages.error(request, 'usuário precisa ser maior que 6 caracteres ou  mais!')
        return render(request, 'accounts/cadastro.html')
